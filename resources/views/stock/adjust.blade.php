<x-app-layout>
    <div class="max-w-lg mx-auto p-4">
        <h1 class="text-xl font-bold text-deep-charcoal mb-4">Koreksi Stok</h1>

        <form method="POST" action="/stock/adjust" class="bg-white border border-light-border rounded-8 p-4 space-y-4 shadow-l1">
            @csrf
            ...
            <div>
                <label class="block font-medium text-sm text-slate-btn mb-1">Jenis Koreksi</label>
                <select name="adjustment_type" class="w-full border border-light-border bg-very-light-gray rounded-6 p-3 h-12 text-deep-charcoal focus:border-slate-btn focus:ring-slate-btn">
                    <option value="adjustment_increase">Tambah Stok</option>
                    <option value="adjustment_decrease">Kurangi Stok</option>
                </select>
            ...
            <div>
                ...
                <input type="text" name="note" value="{{ old('note') }}"
                    class="w-full border border-light-border bg-very-light-gray rounded-6 p-3 h-12 text-deep-charcoal focus:border-slate-btn focus:ring-slate-btn" placeholder="Misal: rusak, hilang, atau koreksi">
                @error('note')
                    <p class="text-error-red text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
