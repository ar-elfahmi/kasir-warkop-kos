<x-app-layout>
    <div class="max-w-lg mx-auto p-4">
        ...
        <form method="POST" action="{{ route('categories.store') }}" class="bg-white rounded-lg shadow p-4 space-y-4">
            @csrf

            <div>
                <label class="block font-medium text-sm mb-1">Nama Kategori</label>
                <input type="text" name="name" value="{{ old('name') }}"
                    class="w-full border rounded-lg p-2" placeholder="Contoh: Minuman">
                @error('name')
